...
distanceData = []  # 2D list for distance matrix
addressData = []    # List of address strings
...
def load_distance_data():
    ...
    with open(file) as fp:
        ...
        distanceData = [row for row in csv.reader(fp)]

#address data loading
def load_address_data():
    global addressData, address_index
    ...
    with open(file) as fp:
        ...
        addressData = [row[2] for row in csv.reader(fp)]
        address_index = {address: index for index, address in enumerate(addressData)}
...
def get_distance(address1, address2):
    ...
    return float(distanceData[max(i,j)][min(i,j)])
...
def min_distance_from(from_address, packages):
    ...
    for package in packages:
        ...
        if distance < min_dist:
            min_dist, nearest_package = distance, package
